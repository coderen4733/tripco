...
# 직무(Duty) 생성(C) API - Redis
async def create_duty_redis(
    redis: Redis,
    field: str,
    value: str,
) -> None:
...
# 직무(Duty) 상세 조회(R-D) API (by _id) - MongoDB
...
# 직무(Duty) 중복 조회(R-D) API (by duty_code) - MongoDB
async def get_duty_by_duty_code(
    db: AsyncIOMotorDatabase,
    duty_code: str,
    _id: str | None,
) -> dict | None:
...
# 직무(Duty) 중복 조회(R-D) API (by name) - MongoDB
...
# 직무(Duty) 가장 마지막 order 조회(R-D) API (by order) - MongoDB
async def get_last_duty_order(
    db: AsyncIOMotorDatabase,
) -> dict | None:
    # 0. 가장 마지막 order 1개만
    cursor = (
...
# 직무(Duty) 수정(U) API - Redis
async def update_duty_redis(
    redis: Redis,
    field: str,
    value: str,
) -> None:
...
# 직무(Duty) 삭제(D) API - Redis
async def delete_duty_redis(
    redis: Redis,
    field: str,
) -> None:
    # 1. Repository => Redis
