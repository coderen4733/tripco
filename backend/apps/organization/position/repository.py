...
async def create_position(
    db: AsyncIOMotorDatabase, position: PositionEntity
) -> str:
    try:
        # 1. Repository => DB
        data = await db[COLLECTION_NAME].insert_one(position.model_dump())
...
# 직급/직위(Positon) 상세 조회(R-D) API (by _id)
...
# 직급/직위(Positon) 중복 조회(R-D) API (by position_id)
async def get_position_by_position_id(
    db: AsyncIOMotorDatabase, position_id: str, _id: str | None
) -> dict | None:
    # 1. Repository => DB
    ...
    if _id is None:
        ...
        data = await db[COLLECTION_NAME].find_one(
            {"_id": {"$ne": ObjectId(_id)}, "position_id": position_id}
        )
    # 2. Repository => Service
    return data


# 직급/직위(Positon) 중복 조회(R-D) API (by name)
...
# 직급/직위(Positon) 가장 마지막 order 조회(R-D) API (by order)
async def get_last_position_order(db: AsyncIOMotorDatabase) -> dict | None:
    # 0. 가장 마지막 order 1개만
    cursor = (
        db[COLLECTION_NAME]
...
async def delete_position(db: AsyncIOMotorDatabase, _id: str) -> dict:
    # 1. Repository => DB
