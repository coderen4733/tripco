...
# 팀(Team) 생성(C) API
async def create_team(db: AsyncIOMotorDatabase, team: TeamEntity) -> str:
    try:
        # 1. Repository => DB
        data = await db[COLLECTION_NAME].insert_one(team.model_dump())
...
# 팀(Team) 상세 조회(R-D) API (by _id)
async def get_team_by_id(db: AsyncIOMotorDatabase, _id: str) -> dict | None:
    # 0. Validation
...
# 팀(Team) 상세 조회(R-D) API (by team_id)
async def get_team_by_team_id(
    db: AsyncIOMotorDatabase, team_id: str, _id: str | None
) -> dict | None:
    # 1. Repository => DB
    ...
    if _id is None:
        data = await db[COLLECTION_NAME].find_one({"team_id": team_id})
    else:
        data = await db[COLLECTION_NAME].find_one(
            {"_id": {"$ne": ObjectId(_id)}, "team_id": team_id}
        )
    # 2. Repository => Service
    return data


# 팀(Team) 상세 조회(R-D) API (by name)
...
# 팀(Team) 가장 마지막 order 조회(R-D) API (by order)
async def get_last_team_order(db: AsyncIOMotorDatabase) -> dict | None:
    # 0. 가장 마지막 order 1개만
    cursor = (
        db[COLLECTION_NAME]
...
# 팀(Team) 삭제(D) API
async def delete_team(db: AsyncIOMotorDatabase, _id: str) -> dict:
    # 1. Repository => DB
